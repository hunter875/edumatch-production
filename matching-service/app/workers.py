...
def process_user_profile_updated(event_data: dict):
    ...
    try:
        ...
        if applicant_feature:
            ...
            applicant_feature.nationality = event.nationality
            applicant_feature.preferred_locations = event.preferredLocations
            applicant_feature.preferred_funding_types = event.preferredFundingTypes
            ...
            applicant_feature = models.ApplicantFeature(
                ...
                preferred_locations=event.preferredLocations,
                preferred_funding_types=event.preferredFundingTypes,
                skills=event.skills,
...
def process_scholarship_created(event_data: dict):
    ...
    try:
        ...
        opportunity_feature = models.OpportunityFeature(
            ...
            eligible_majors=event.eligibleMajors,
            eligible_nationalities=event.eligibleNationalities,
...
def process_scholarship_updated(event_data: dict):
    ...
    try:
        ...
        if opportunity_feature:
            ...
            opportunity_feature.eligible_majors = event.eligibleMajors
            opportunity_feature.eligible_nationalities = event.eligibleNationalities
...
def publish_match_notification(applicant_id: int, opportunity_id: str, score: float, title: str):
    """
    Publish match notification to RabbitMQ with dedup.
    Dedup key: applicant_id + opportunity_id + notification type.
    """
    connection = None
    try:
        # Dedup check — don't send the same match notification twice
        dedup_key = f"match:{applicant_id}:{opportunity_id}"
        # Use a simple in-memory dedup cache with TTL (lives as long as the worker process)
        if not hasattr(publish_match_notification, '_sent'):
            publish_match_notification._sent = {}
        # Clean old entries (> 1 hour)
        now = datetime.utcnow()
        publish_match_notification._sent = {
            k: v for k, v in publish_match_notification._sent.items()
            if (now - v).total_seconds() < 3600
        }
        if dedup_key in publish_match_notification._sent:
            logger.info(
                "Dedup: skipping duplicate notification for user=%s opportunity=%s",
                applicant_id, opportunity_id,
            )
            return
        publish_match_notification._sent[dedup_key] = now

        logger.info("Sending match notification to User %s...", applicant_id)
        ...
        channel.basic_publish(
            ...
            properties=pika.BasicProperties(
                ...
                message_id=dedup_key,  # helps downstream dedup too
            ),
        )

        logger.info(
            "Sent match notification for User %s (Score: %.1f%%)",
            applicant_id, score,
        )

    except Exception as e:
        logger.error(
            "Failed to publish notification to User %s: %s", applicant_id, e, exc_info=True,
        )
    finally:
